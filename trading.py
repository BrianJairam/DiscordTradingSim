...
def add_to_portfolio(user_id, stock_name, amount):
    ...
    cursor = db.cursor()
    cursor.execute('SELECT amount FROM stocks WHERE user_id = ? AND stock = ?',
                   (user_id, stock))
...
def stock_ledger_update(transaction_type, guild_id, user_id,
                        stock_name, stock_price, number):
    db = sqlite3.connect('main.sqlite')
    cursor = db.cursor()
    ledger_sql = ('INSERT INTO stock_ledger \
                  (transaction_type, guild_id, user_id, \
                  stock, stock_price, number, payment, date) \
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    ledger_val = (transaction_type, guild_id, user_id,
                  f"\"{stock_name.upper()}\"", stock_price,
                  number, stock_price * number,
...
def check_portfolio(user):
    db = sqlite3.connect('main.sqlite')
    cursor = db.cursor()
    cursor.execute('SELECT stock, amount FROM stocks WHERE user_id = ? \
                    ORDER BY amount DESC', (user.id,))
    ...
    embed = discord.Embed(title=f"Portfolio of {user.name}",
                          description="Shares | Price Per Share | Total Value",
                          color=0x00fa00)
...
def buy_history(user):
    db = sqlite3.connect('main.sqlite')
    cursor = db.cursor()
    cursor.execute('SELECT * FROM stock_ledger WHERE user_id = ? \
                    AND transaction_type = \"\"\"Buy Order\"\"\"',
                   (user.id,))
    rows = cursor.fetchall()
    ...
    for row in rows:
        ...
        date = row[8]
        embed.add_field(name=stock_name,
                        value=f"{amount} | {price:.2f} | {value:.2f} | {date}",
                        inline=False)
...
def sell_history(user):
    ...
    cursor = db.cursor()
    cursor.execute('SELECT * FROM stock_ledger WHERE user_id = ? \
                    AND transaction_type = \"\"\"Sell Order\"\"\"',
                   (user.id,))
    rows = cursor.fetchall()
...
def order_history(user):
    ...
    for row in rows:
        ...
        value = row[7]
        date = row[8]
        embed.add_field(name=order_type + stock_name,
                        value=f"{amount} | {price:.2f} | {value:.2f} | {date}",
                        inline=False)
    embed.set_footer(text=f"Sell Orders")
    return embed
...
def portfolio_history(user, start, inc):
    ...
    if (start == "begin"):
        start = datetime.datetime.strptime(ledger_data[0][8],
                                           '%Y-%m-%d %H:%M:%S').date()
    stock_history = []
    histories = []
    ...
    for row in ledger_data:
        ...
        if (stock not in stock_history):
            ...
            print(end_date)
            print(inc)
            yf_stock_data = yf.download(stock,
                                        start=start_date,
                                        end=end_date,
                                        interval=inc)
    ...
    values = []
    cur_date = start
    # Loop through dates, calculting value of portfolio at any given date
    # (upto but not including the end date)
    while (cur_date < end):
        if (i <= max_i):
            while (cur_date >=
                   datetime.datetime.strptime(ledger_data[i][8],
                                              '%Y-%m-%d %H:%M:%S').date()):
        ...
        for stock in stocks:
            number = stocks[stock]
            stock_index = stock_history.index(stock)
            last_date = cur_date
            while (last_date.strftime("%Y-%m-%d") not in
                   histories[stock_index]["Close"]):
                last_date -= datetime.timedelta(days=1)
            last_time = last_date.strftime("%Y-%m-%d")
            value += histories[stock_index]["Close"][last_time] * number
