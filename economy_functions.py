...
def ledger_update(transaction_type, guild_id, giver_id, receiver_id, payment):
    ...
    db = sqlite3.connect('main.sqlite')
    cursor = db.cursor()
    ledger_sql = ('INSERT INTO ledger (transaction_type, guild_id, giver_id, \
                                       receiver_id, dollars, giver_balance, \
                                       receiver_balance, date) \
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    ledger_val = (transaction_type, guild_id, giver_id,
                  receiver_id, payment,
                  gbalance, rbalance,
